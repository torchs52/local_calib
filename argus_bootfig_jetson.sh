...
#
# flock を使用するため、VS Code が argus_bootfig_jetson.sh を開いているだけでは二重起動とは判定されない。
# argus_bootfig.sh / argus_bootfig_jetson.sh で共通ロックを使用するため、ロックファイル名は argus3d.lock とする。
# ----------------------------------------------------------------------
...
if ! flock -n 9; then
    ...
    # --------------------------------------------------------------
    # 前回の argus_bootfig が cleanup を完了し、
    ...
    #
    # 新しい側から bootfig 自体を SIGKILL すると、
    # cleanup を飛ばしてプロセスや MMAP を残す可能性があるため、
    # ここでは起動を中止する。
    # --------------------------------------------------------------
